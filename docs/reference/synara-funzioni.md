...
**Percorsi dei file.** `provider/Layers/CursorAdapter.ts` (1919 righe); `provider/acp/CursorAcpCommand.ts`, `CursorAcpSupport.ts`, `CursorAcpExtension.ts`, `CursorAcpCliProbe.ts`; `provider/cursorSkillsDiscovery.ts`; controllo di accesso in `provider/Layers/ProviderHealth.ts:1585-1787`.
...
**Capacità dichiarate.** `sessionModelSwitch: "restart-session"`, `conversationRollback: "restart-session"`, `supportsRuntimeModelList: true`, `supportsLiveTurnDiffPatch: false` (`AntigravityAdapter.ts:2764-2769`). Composer: skill sì, plugin no, comandi nativi no, compattazione no, import no (`:2783-2794`).
