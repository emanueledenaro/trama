...
| --- | --- | --- |
| T01 | Shell SwiftUI nativa, apertura progetto, catalogo dei recenti con bookmark, ripristino dello stato e scanner in sola lettura. Sono presenti test per identità, spostamento e catalogo corrotto. | Prova manuale di primo avvio, selettore annullato, cartella vuota, permesso negato, riavvio, tastiera e VoiceOver. Manca una CI remota eseguita. |
| T02 | `CodexClient` usa Codex App Server 0.148.0, riconosce account ChatGPT, gestisce login, timeout, JSON errato e uscita del processo. I test usano un trasporto simulato. | Turno modello minimo reale con isolamento confermato. Restano da provare login annullato o scaduto e percorso di riparazione su un Mac senza Codex. |
...
| T11 | Lettura e creazione issue, preparazione di un commit immutabile, anteprima e creazione PR con controllo di base e head. Sono presenti test per timeout, riconciliazione e cambio candidato. | Prova autenticata su GitHub con anteprima UI, push e PR reali. CI remota, merge e chiusura issue devono restare osservati come passaggi distinti. |
...
| T18 | Repository pubblico, workflow macOS preparato, script Release, risorse incluse e firma ad hoc locale. | Pubblicazione del sorgente corrente, clone pulito, esecuzione CI remota, licenza della radice, termini di redistribuzione Codex, Developer ID, notarizzazione e installazione su un secondo Mac. |
...
La dicitura beta locale richiede almeno un clone pulito del codice pubblicato, build Release, test sullo stesso commit, avvio dell'app e un percorso manuale principale completato. La distribuzione scaricabile richiede anche licenza della radice, verifica dei termini Codex, firma Developer ID, notarizzazione e installazione su un altro Mac. Queste condizioni non risultano ancora soddisfatte.
