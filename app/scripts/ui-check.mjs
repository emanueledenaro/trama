...
// Team: asking about a specialist names its latest assignment.
await page.getByRole("button", { name: /^Team/ }).first().click();
await page.getByTestId("inspector").getByRole("button", { name: /^Ada/ }).click();
await page.getByTestId("inspector").getByRole("button", { name: "Chiedi al Coordinatore", exact: true }).click();
await expectAsked("di Ada", "Team, Chiedi al Coordinatore");
if (!(await composer().inputValue()).includes("Aggiornami sull'incarico A-")) throw new Error("The question does not name Ada's assignment");
